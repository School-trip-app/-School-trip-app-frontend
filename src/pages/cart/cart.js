...
function Cart() {
  ...
  const getPackage = async () => {
    await axios.get('http://localhost:4006/package').then((res) => {
  ...
  const getPhotoger = async (id) => {
    await axios.get(`http://localhost:4006/photographer/${id}`).then(res => {
      setphoter(res.data);
  ...
  const getProduct = async () => {
    try {
      if (state.prodectId.length !== 0) {
        await axios.get(`http://localhost:4006/product`).then(res => {
          setprodct(res.data);
        });
      }
    } catch (err) {
    ...
    try {
      if (state.prodectId.length !== 0) {
        for (let i = 0; i < state.prodectId?.length; i++) { 
           await axios.get(`http://localhost:4006/product/${state.prodectId[i]}`).then(res=>{
  ...
  const sentOrder = async () => {
    ...
    console.log(order);
    await axios.post(`http://localhost:4006/package/order/${cookies.load('userId')}/${state.packId}`, order).then((res) => {
